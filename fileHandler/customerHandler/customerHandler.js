    ...
    if (!obj) {
        ...
        commonFile.jwtDecode(obj.token, (err, result) => {
            ...
            if (err) {
                ...
                console.log(result)
                signup_petModel.findOne({customer_Id:result},{_id:0}).populate({path:"customer_Id",select:"name"}).exec((err,success)=>{
                    if (err) {
                        Response.sendResponsewithError(res, resCode.INTERNAL_SERVER_ERROR, resMessage.INTERNAL_SERVER_ERROR, err)
                    }
                    else if (!result) {
                        ...
                        Response.sendResponseWithData(res,resCode.EVERYTHING_IS_OK,resMessage.EVERYTHING_IS_OK,success)
                    }
                })
            }
        })
    ...
    if (!obj) {
        ...
        commonFile.jwtDecode(obj.token, (err, result) => {
            ...
            if (err) {
                ...
                console.log(result)
                signup_petModel.findOne({customer_Id:result}).populate({path:"customer_Id"}).exec((err,success)=>{
                    if (err) {
                        Response.sendResponsewithError(res, resCode.INTERNAL_SERVER_ERROR, resMessage.INTERNAL_SERVER_ERROR, err)
                    }
                    else if (!result) {
